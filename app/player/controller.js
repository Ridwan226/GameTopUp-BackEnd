...
module.exports = {
  ...
  checkout: async (req, res) => {
    try {
      const {accountUser, name, voucher, payment, bank, nominal} = req.body;
      ...
      if (!res_nominal)
        return res.status(404).json({message: `nominal Game Tidak di Temukan`});

      const res_payment = await Payment.findOne({_id: payment});
      if (!res_payment)
        return res.status(404).json({message: `payment Game Tidak di Temukan`});

      const res_bank = await Bank.findOne({_id: bank});
      if (!res_bank)
        return res.status(404).json({message: `bank Game Tidak di Temukan`});
  ...
  historyDetail: async (req, res) => {
    try {
      ...
      if (!history) {
        return res.status(404).json({message: `history Game Tidak di Temukan`});
      }
  ...
  dashboard: async (req, res) => {
    try {
      ...
      const history = await Transaction.find({player: req.player._id}).populate(
        "category",
      );
  ...
  editProfile: async (req, res, next) => {
    try {
      ...
      if (phoneNumber.length) phoneNumber.phoneNumber = phoneNumber;
      ...
      if (req.file) {
        ...
        src.on("end", async () => {
          try {
            ...
            res
              .status(500)
              .json({message: `Terjadi kesalahan pada server Upload`});
          }
        });
      } else {
